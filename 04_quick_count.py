    ...
    for filename in glob.glob(f'{args.data_dir}/*/diffs.json'):
        with open(filename, 'r') as f:
            ...
            #for diff in obj['diffs']:
                #old, new = un_none(diff)
                #print(f'{len(old)//10}\t{len(new)//10}')
            diff_counter.update(count_diffs(obj, identifier))
            #sentence_lookup = build_sentence_lookup(obj['tokens']['initial'])
            #for diff in obj['diffs']:
            #    print_contextualized(diff, obj['tokens']['initial'],
            #    sentence_lookup)

    #total_diffs = sum(len(i) for i in diff_counter.values())
    #for k, v in diff_counter.items():
    #    if k == Categories.OTHER:
    #        continue
    #    else:
    #        for x in v[:20]:
    #            print(k, x)
    #for k, v in diff_counter.items():
    #    print(f'{k}\t{len(v)*100/total_diffs:.2f}%')
