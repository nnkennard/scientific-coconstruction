...
def get_reviews(forum_notes):
    ...
    for note in forum_notes:
        if note.replyto == note.forum and "main_review" in note.content:
...
def jsonify_forum(forum):
    ...
    return json.dumps(forum_dict)


def get_tsv_path(n_batches, offset, output_dir, conference, i=None):
    prefix = f"{output_dir}/metadata_{conference}"
    if n_batches == 1:
        assert i == 0
        if offset == 0:
            return f"{prefix}.tsv"
        else:
            return f"{prefix}_offset{offset}.tsv"
    else:
        assert i is not None
        if offset == 0:
            return f"{prefix}_{str(i).zfill(4)}.tsv"
        else:
            return f"{prefix}_offset{offset}_{str(i).zfill(4)}.tsv"
